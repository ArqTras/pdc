...
#define WALLET_TRANSFER_INFO_FLAGS_HTLC_DEPOSIT   static_cast<uint16_t>(1 << 0)
  ...
  struct wallet_transfer_info
  {
    ...
    BEGIN_KV_SERIALIZE_MAP()
      ...
      KV_SERIALIZE(is_service)                    DOC_DSCR("Tells if this transaction is used as utility by one of Pdc services(contracts, ionic swaps, etc)")  DOC_EXMP(false)   DOC_END
  ...
  struct COMMAND_RPC_TRANSFER
  {
    ...
    struct request
    {
      ...
      BEGIN_KV_SERIALIZE_MAP()
        ...
        KV_SERIALIZE(service_entries)  DOC_DSCR("Service entries that might be used by different apps that works on top of Pdc network, not part of consensus") DOC_EXMP_AUTO(1)     DOC_END
