...
bool wallet2::prepare_and_sign_pos_block(const mining_context& cxt, uint64_t full_block_reward, const currency::pos_entry& pe, currency::tx_generation_context& miner_tx_tgc, currency::block& b) const
{
  ...
  if (!cxt.zarcanum)
  {
    ...
    if (m_required_decoys_count > 0 && !is_auditable() && stake_allows_mixins)
    {
      ...
      // max_related <= block_height - min_coinstake_age - 1 (== daemon_height - min_coinstake_age - 1 when synced)
      decoys_req.height_upper_limit = std::min(m_last_pow_block_h,
        m_last_known_daemon_height > m_core_runtime_config.min_coinstake_age + 1
          ? m_last_known_daemon_height - m_core_runtime_config.min_coinstake_age - 1
          : m_last_pow_block_h);
  ...
  std::string* p_unsigned_filename_or_tx_blob_str)
{
  ...
  if (m_watch_only)
  {
    bool r = store_unsigned_tx_to_file_and_reserve_transfers(ftp, (p_unsigned_filename_or_tx_blob_str != nullptr ? *p_unsigned_filename_or_tx_blob_str : "pdc_tx_unsigned"), p_unsigned_filename_or_tx_blob_str);
