  ...
  m_offline_mode(false)
{
  ...
  m_cmd_binder.set_handler("list_outputs", boost::bind(&simple_wallet::list_outputs, this, ph::_1), "list_outputs [spent|unspent] [ticker=PDC] [unknown] - Lists all the outputs. The result may be filtered by spent status, asset ticker or unknown asset ids.");
...
bool simple_wallet::validate_wrap_status(uint64_t amount)
{
  ...
  if (amount > unwrapped_coins_left)
  {
    fail_msg_writer() << "Amount is bigger than ERC20 tokens left available: "
      << print_money(unwrapped_coins_left) << " wPdc";
    return false;
  }
...
bool simple_wallet::transfer(const std::vector<std::string> &args_)
{
  ...
  for (size_t i = 0; i < local_args.size(); i += 2) 
  {
    ...
    if (is_address_like_wrapped(local_args[i]))
    {
      ...
      success_msg_writer(false) << "This transaction will create wPdc (\"Wrapped Pdc\") which will be sent to the specified address on the Ethereum network.";
  ...
  if (!m_wallet->is_watch_only())
  {
    if(wrapped_transaction)
      success_msg_writer(true) << "Transaction successfully sent to wPdc custody wallet, id: " << get_transaction_hash(tx) << ", " << get_object_blobsize(tx) << " bytes";
    else
      success_msg_writer(true) << "Transaction successfully sent, id: " << get_transaction_hash(tx) << ", " << get_object_blobsize(tx) << " bytes";
  }
  else
  {
    success_msg_writer(true) << "Transaction prepared for signing and saved into \"pdc_tx_unsigned\" file, use full wallet to sign transfer and then use \"submit_transfer\" on this wallet to broadcast the transaction to the network";
  }  
...
bool simple_wallet::sweep_below(const std::vector<std::string> &args)
{
  ...
  currency::transaction result_tx = AUTO_VAL_INIT(result_tx);
  std::string filename = "pdc_tx_unsigned";
...
bool simple_wallet::sweep_bare_outs(const std::vector<std::string> &args)
{
  ...
  if (groups.empty())
  {
    ...
    if (balance < COIN)
      success_msg_writer(false) << "Looks like it's not enough coins to perform this operation. Transferring " << print_money_brief(TX_MINIMUM_FEE) << " PDC or more to this wallet may help.";
