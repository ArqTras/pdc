...
SET ACHIVE_NAME_PREFIX=pdc-win-x64-
SET MY_PATH=%~dp0
...
msbuild src/Pdc.vcxproj /p:SubSystem="WINDOWS,5.02" /p:Configuration=Release /t:Build
...
call :sign_file Pdc.exe || goto error
call :sign_file pdcd.exe || goto error
...
copy /Y Pdc.exe bunch
copy /Y pdcd.exe bunch
copy /Y simplewallet.exe bunch
copy /Y *.pdb bunch

%QT_MSVC_PATH%\bin\windeployqt.exe bunch\Pdc.exe || goto error
...
pscp -load pdc_build_server %installer_path% %PDC_BUILDS_HOST%:/var/www/html/builds
IF %ERRORLEVEL% NEQ 0 (
...
pscp -load pdc_build_server %build_zip_path% %PDC_BUILDS_HOST%:/var/www/html/builds
IF %ERRORLEVEL% NEQ 0 (
...
python ../utils/build_mail.py "Pdc win-x64 %build_prefix% %TESTNET_LABEL%build %version%" "%emails%" %mail_msg%
